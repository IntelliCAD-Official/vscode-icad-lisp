...
export async function findInProject() {
    ...
    if (ProjectTreeProvider.hasProjectOpened() == false) {
        let msg = localize("icad-lisp.project.find.openproject", "A project must be open before you can search for a text string.");
        vscode.window.showInformationMessage(msg);
        return;
    }

    //get search option
    let title = localize("icad-lisp.project.find.title", "Find in Project");
    let hint = localize("icad-lisp.project.find.hint", "Type a text string to find, and press Enter.");
...
export class FindInProject {
    ...
    public async execute(searchOption: SearchOption, prjNode: ProjectNode) {
        if (os.platform() == 'win32') {
            if (os.arch() != 'x64') {
                let msg = localize("icad-lisp.project.find.supportos", "Find & Replace is supported only on 64-bit systems.");
        ...
        try {
            ...
            let summary = localize("icad-lisp.project.find.inprogress", "In progress... ");
            ...
            let found = localize("icad-lisp.project.find.found", "Found ");
            let lines = localize("icad-lisp.project.find.results", " result(s) in ");
            let files = localize("icad-lisp.project.find.files", " file(s):");
            ...
            if (SearchOption.activeInstance.stopRequested) {
                this.summaryNode.summary = localize("icad-lisp.project.find.stopped", "Find stopped. ");
            } else {
                this.summaryNode.summary = '';
            }

            if (exceedMaxResults) {
                this.summaryNode.summary += localize("icad-lisp.project.find.exceedmaxresults", "Limit of search results exceeded. Refine your search to narrow down the results. ");
            }

            if (totalLines <= 0) {
                if (!exceedMaxResults) {
                    this.summaryNode.summary += localize("icad-lisp.project.find.noresults", "No results found.");
                }
            }
            else {
